...
async def run_trial_async(
    ...
    try:
        ...
        # Handle image differencing for initial state. The initial scene
        # description may use the policy context, but per-turn differencing
        # should only receive the clean task/API prompt.
        task_description_with_skills = copy.deepcopy(
            obs["full_prompt"][-1]["content"][0]["text"]
        )
        task_description = task_only_prompt or strip_external_skills_from_prompt(
            task_description_with_skills
        )
        if use_img_differencing and initial_visual_feedback_base64:
            ...
            initial_env_description_prompt = [
                ...
                },
                {
                    "role": "user",
                    "content": [
                        {"type": "text", "text": task_description_with_skills},
