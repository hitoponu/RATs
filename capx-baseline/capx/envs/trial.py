...
from capx.skills.planner import run_external_skill_planner
from capx.skills.rats_adapter import strip_external_skills_from_prompt
...
def _append_policy_prompt_text(obs: dict[str, Any], text: str) -> None:
    """Append text context to the main policy prompt."""
    if not text:
        return
    user_content = obs["full_prompt"][-1]["content"]
    if user_content and isinstance(user_content[0], dict) and "text" in user_content[0]:
        user_content[0]["text"] += "\n\n" + text
    else:
        user_content.insert(0, {"type": "text", "text": text})


def _maybe_apply_external_skill_planner(
    args: LaunchArgs,
    env: CodeExecutionEnvBase,
    obs: dict[str, Any],
) -> dict[str, Any] | None:
    """Run the external skill planner and append selected skills to the policy prompt."""
    env_cfg = getattr(env, "cfg", None)
    if env_cfg is None or not getattr(env_cfg, "external_skill_library_path", None):
        return None

    mode = str(getattr(env_cfg, "external_skill_library_mode", "direct") or "direct").strip().lower()
    if mode != "planner":
        return None

    task_context = strip_external_skills_from_prompt(
        copy.deepcopy(obs["full_prompt"][-1]["content"][0]["text"])
    )
    result = run_external_skill_planner(
        args=args,
        task_context=task_context,
        skill_library_path=env_cfg.external_skill_library_path,
        max_library_skills=env_cfg.external_skill_library_max_skills,
        include_primitives=env_cfg.external_skill_library_include_primitives,
        planner_include_code=env_cfg.external_skill_planner_include_code,
        policy_include_code=env_cfg.external_skill_policy_include_code,
        max_selected=env_cfg.external_skill_planner_max_selected,
        planner_model=env_cfg.external_skill_planner_model,
    )

    selected_prompt = result.get("selected_skills_prompt") or ""
    if selected_prompt:
        _append_policy_prompt_text(obs, selected_prompt)
        print(f"[SkillPlanner] Selected {result.get('selected_skill_count', 0)} skill(s)")
    elif result.get("error"):
        print(f"[SkillPlanner] Skill selection failed: {result['error']}")
    else:
        print("[SkillPlanner] Selected 0 skill(s)")
...
def _handle_multi_turn_step(
    ...
    wrist_turn_frames: list[np.ndarray] | None = None,
    wrist_base64_history: list[str] | None = None,
) -> tuple[str, str | None, str | None, dict | None, list | None, str | None, str | None]:
    ...
    Returns:
        (decision, new_code, reasoning, multiturn_ensemble_entry, decision_prompt,
        differencing_feedback, differencing_feedback_type)
    ...
    differencing_feedback_type = None
    if differencing_feedback is not None:
        differencing_feedback_type = "video" if is_video_feedback else "image"
...
def _run_single_trial(
    ...
    skill_planner_result: dict[str, Any] | None = None
    ...
    skill_planner_result = _maybe_apply_external_skill_planner(args, env, obs)
    ...
    if skill_planner_result is not None:
        initial_response.update({
            "skill_planner_prompt": (
                skill_planner_result.get("planner_prompt")
                if config.get("save_skill_planner_prompts", False)
                else None
            ),
            "skill_planner_response": skill_planner_result.get("planner_response"),
            "skill_planner_selected_skill_ids": skill_planner_result.get("selected_skill_ids", []),
            "skill_planner_selected_skill_count": skill_planner_result.get("selected_skill_count", 0),
            "skill_planner_rationale": skill_planner_result.get("rationale", ""),
            "skill_planner_error": skill_planner_result.get("error"),
            "selected_skills_prompt": (
                skill_planner_result.get("selected_skills_prompt")
                if config.get("save_skill_planner_prompts", False)
                else None
            ),
        })
        ...
        if multi_turn_prompt:
            ...
            if decision == "regenerate":
                ...
                all_responses.append({
                    ...
                    "visual_differencing_feedback": differencing_feedback,
                    "visual_differencing_feedback_type": differencing_feedback_type,
...
def _patch_libero_goal(env: CodeExecutionEnvBase, obs: dict[str, Any]) -> None:
    ...
    placeholder = "{libero_environment_goal}"
    if (
        hasattr(handle, "task_language")
        and placeholder in obs["full_prompt"][-1]["content"][0]["text"]
    ):
        goal = str(getattr(handle, "task_language"))
        obs["full_prompt"][-1]["content"][0]["text"] = (
            obs["full_prompt"][-1]["content"][0]["text"].replace(placeholder, goal)
        )
