...
def _load_config(args: LaunchArgs) -> tuple[Any, dict[str, Any], list]:
    ...
    merged_config = {
        ...
        "save_skill_planner_prompts": configs_dict.get("save_skill_planner_prompts", False),
...
def _save_trial_artifacts(
    ...
    for response in all_responses:
        ...
        if response.get("skill_planner_prompt") is not None:
            (trial_dir / "prompts_and_responses" / "skill_planner_prompt.txt").write_text(
                str(response["skill_planner_prompt"])
            )
        if (
            "skill_planner_response" in response
            or "skill_planner_selected_skill_ids" in response
            or "skill_planner_error" in response
        ):
            planner_response = {
                "response": response.get("skill_planner_response"),
                "selected_skill_ids": response.get("skill_planner_selected_skill_ids", []),
                "selected_skill_count": response.get("skill_planner_selected_skill_count", 0),
                "rationale": response.get("skill_planner_rationale", ""),
                "error": response.get("skill_planner_error"),
            }
            (trial_dir / "prompts_and_responses" / "skill_planner_response.json").write_text(
                json.dumps(planner_response, indent=2)
            )
        if response.get("selected_skills_prompt") is not None:
            (trial_dir / "prompts_and_responses" / "selected_skills_prompt.txt").write_text(
                str(response["selected_skills_prompt"])
            )
        if "multi_turn_prompt" in response and response["multi_turn_prompt"] is not None:
            ...
            if response.get("visual_differencing_feedback") is not None:
                feedback_type = response.get("visual_differencing_feedback_type") or "unknown"
                (trial_dir / "prompts_and_responses" / f"visual_differencing_feedback_{i:02d}.txt").write_text(
                    f"type: {feedback_type}\n\n{response['visual_differencing_feedback']}"
                )
            i += 1
        elif response.get("visual_differencing_feedback") is not None:
            feedback_type = response.get("visual_differencing_feedback_type") or "unknown"
            (trial_dir / "prompts_and_responses" / f"visual_differencing_feedback_{i:02d}.txt").write_text(
                f"type: {feedback_type}\n\n{response['visual_differencing_feedback']}"
            )
