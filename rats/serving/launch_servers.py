...
Start all API servers needed for an environment config with one command:
    ...
    uv run --no-sync --active rats/serving/launch_servers.py \
        --config-path env_configs/cube_stack/franka_robosuite_cube_stack.yaml

Or start a predefined server profile:

    uv run --no-sync --active rats/serving/launch_servers.py --profile default
    uv run --no-sync --active rats/serving/launch_servers.py --profile full
"""
...
SERVER_REGISTRY: dict[str, dict[str, Any]] = {
    "sam3": {
        "target": "rats.serving.launch_sam3_server",
        "default_port": 8114,
        "gpu_required": True,
        "gpu_memory_mb": 3000,
    ...
    "graspnet": {
        "target": "rats.serving.launch_contact_graspnet_server",
        "default_port": 8115,
        "gpu_required": True,
        "gpu_memory_mb": 2000,
        "extra_args": {"device": "cuda"},
    ...
    "pyroki": {
        "target": "rats.serving.launch_pyroki_server",
        "default_port": 8116,
        "gpu_required": False,
        "gpu_memory_mb": 0,
        "extra_args": {},
    ...
    "sam2": {
        "target": "rats.serving.launch_sam2_server",
        "default_port": 8113,
        "gpu_required": True,
        "gpu_memory_mb": 6000,
        "extra_args": {"device": "cuda"},
    ...
    "owlvit": {
        "target": "rats.serving.launch_owlvit_server",
        "default_port": 8118,
        "gpu_required": True,
        "gpu_memory_mb": 3000,
        "extra_args": {"device": "cuda"},
    ...
    "curobo": {
        "target": "rats.serving.launch_curobo_server",
        "default_port": 8117,
        "gpu_required": True,
        "gpu_memory_mb": 2000,
        "extra_args": {},
...
def parse_servers_from_yaml(config_path: str) -> list[dict[str, Any]]:
    ...
    The YAML ``api_servers`` key contains entries like::
        ...
        api_servers:
          - _target_: rats.serving.launch_sam3_server.main
            device: cuda
            port: 8114
            host: 127.0.0.1
