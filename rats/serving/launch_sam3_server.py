...
def _set_torch_cuda_device(device: str) -> None:
    """Make the requested CUDA device the active torch device.

    Some SAM3 internals create tensors on the current default CUDA device rather than
    always following the model's parameter device, so we pin the current device before
    model construction and again before each inference call.
    """
...
def _do_segment(pil_image: Image.Image, text_prompt: str):
    ...
    _set_torch_cuda_device(_DEVICE)
    dtype_context = (
