...
import rats.integrations.motion.pyroki_snippets as pks
...
class IkRequest(BaseModel):
    ...
    ``obstacles`` opts into ``solve_ik_with_collision`` instead of the
    basic non-collision solver. List entries match ObstacleEntry below
    (handled by ``_build_world_coll``). When omitted/empty, behavior is
    backwards-compatible (basic solve_ik).
    """
...
class PlanRequest(BaseModel):
    ...
    timesteps: int = 20
    dt: float = 0.02
    start_cfg: list[float] | None = None  # optional joint config (skip IK re-solve)
    end_cfg: list[float] | None = None  # optional joint config (skip IK re-solve)
...
def _do_solve_ik(
    ...
    When ``obstacles`` is a non-empty list AND ``_ROBOT_COLL`` is loaded,
    routes through ``solve_ik_with_collision`` so the returned joint
    configuration avoids both self-collisions and the provided world
    geometry. Otherwise falls back to the basic / vel-cost solver.
    """
    world_coll = _build_world_coll(obstacles) if obstacles else []
    if world_coll and _ROBOT_COLL is not None:
        q = pks.solve_ik_with_collision(
            robot=_ROBOT,
            coll=_ROBOT_COLL,
...
async def solve_ik(req: IkRequest):
    ...
    try:
        joints = await _run_in_thread(
            _do_solve_ik, target_pose_wxyz_xyz, prev_cfg, req.obstacles,
        )
...
def _do_plan_motion(req: PlanRequest) -> PlanResponse:
    ...
    When ``req.obstacles`` is non-empty AND ``_ROBOT_COLL`` is loaded,
    routes through ``pks.solve_trajopt`` which optimizes the full
    waypoint sequence subject to robot self-collision + world-collision
    + smoothness costs. Otherwise falls back to the legacy
    ``plan_trajectory_linear_ik`` (linear pose interpolation + per-
    waypoint IK), which is what every prior caller used.
    """
    ...
    world_coll = _build_world_coll(req.obstacles) if req.obstacles else []
    if world_coll and _ROBOT_COLL is not None:
        start_cfg_arr = np.array(req.start_cfg, dtype=np.float64) if req.start_cfg else None
        end_cfg_arr = np.array(req.end_cfg, dtype=np.float64) if req.end_cfg else None
        sol_traj = pks.solve_trajopt(
            robot=_ROBOT,
            ...
            timesteps=timesteps,
            dt=dt,
            start_cfg=start_cfg_arr,
            end_cfg=end_cfg_arr,
        )
        sol_traj = np.asarray(sol_traj)
        logger.info(
            "/plan used solve_trajopt with %d obstacles, %d waypoints",
            len(world_coll), timesteps,
        )
    else:
