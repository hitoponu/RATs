...
from rats.web.models import SessionState, WSEventBase

if TYPE_CHECKING:
    from rats.web.async_trial_runner import TrialContext

logger = logging.getLogger(__name__)


def _close_session_env(env: Any) -> None:
    """Best-effort environment shutdown used by session cleanup paths."""
    if env is None:
        return
...
def _stop_session_api_servers(server_procs: list[Any]) -> None:
    """Best-effort API server shutdown used by session cleanup paths."""
    if not server_procs:
        return
    from rats.envs.runner import _stop_api_servers
...
class Session:
    ...
    env: Any = None
    api_server_procs: list[Any] = field(default_factory=list)
    ...
    async def emit(self, event: WSEventBase) -> None:
        ...
        # Store for replay on reconnect (skip transient/high-frequency events)
        if event.type not in {"model_streaming_delta", "viser_reload"}:
...
class SessionManager:
    ...
    async def _cleanup_session_unlocked(self, session_id: str) -> None:
        ...
        with contextlib.suppress(Exception):
            _close_session_env(session.env)
        session.env = None

        with contextlib.suppress(Exception):
            _stop_session_api_servers(session.api_server_procs)
        session.api_server_procs = []
    ...
    async def stop_session(self, session_id: str) -> bool:
        ...
        if session.task and not session.task.done():
            ...
            if session.api_server_procs:
                try:
                    _stop_session_api_servers(session.api_server_procs)
                except Exception as e:
                    logger.warning(f"Error stopping API servers: {e}")
                session.api_server_procs = []
    ...
    def get_active_session(self) -> Session | None:
        ...
        the single session if it exists and is still running. Read-only
        historical RATS sessions are also returned so the frontend can
        reconnect after refresh.
        """
        for session in self._sessions.values():
            if session.config.get("rats_history"):
                return session
