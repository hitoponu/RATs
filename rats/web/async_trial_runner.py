...
from rats.utils.video_utils import _write_video
...
from rats.web.session_manager import Session, _close_session_env, run_blocking_with_interrupt
...
async def run_trial_async(
    ...
    try:
        ...
        from rats.envs.trial import _patch_libero_goal
        ...
        while code_block_idx < len(code_blocks) and code_block_idx <= MULTITURN_LIMIT:
            ...
            if multi_turn_prompt:
                ...
                # Build multi-turn prompt — pass task_goal so templates that
                # reference {task_goal} (e.g. "REMINDER — original task goal:
                # {task_goal}") don't KeyError. .format() ignores extra kwargs
                # that aren't referenced, so templates without {task_goal}
                # remain unaffected.
                _task_goal_for_fmt = task_description if isinstance(task_description, str) else ""
                complete_multi_turn_prompt = multi_turn_prompt.format(
                    executed_code=executed_code,
                    console_stdout=info_step["stdout"],
                    console_stderr=info_step["stderr"],
                    task_goal=_task_goal_for_fmt,
                )
        ...
        if env is not None:
            with contextlib.suppress(Exception):
                await asyncio.to_thread(_close_session_env, env)
        session.env = None
