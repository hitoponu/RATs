...
class ExecutionStepEvent(WSEventBase):
    ...
    camera captures, etc. during code execution. Optional frame/second
    offsets line the step up with MolmoSpaces attempt videos.
    """
...
class ViserReloadEvent(WSEventBase):
    """Emitted when the active environment/Viser server changes."""

    type: str = "viser_reload"
    port: int | None = None
    reason: str = "env_changed"
...
class ViserReloadEvent(WSEventBase):
    """Emitted when the active Viser backend changes and the iframe should reload."""
