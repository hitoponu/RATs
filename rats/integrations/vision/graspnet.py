...
def _normalize_point_cloud_for_service(
    points: np.ndarray,
    name: str,
) -> tuple[np.ndarray, dict[str, Any]]:
    """Return a finite (N, 3) point cloud for the GraspNet service.

    MolmoSpaces helper APIs document point clouds as (N, 3), but policy code
    can sometimes preserve image-shaped clouds or transpose a sampled cloud to
    (3, N). ContactGraspNet indexes the second axis as xyz, so passing (3, N)
    can surface as an opaque boolean-index shape mismatch inside the server.
    """
    arr = np.asarray(points)
    diagnostics: dict[str, Any] = {
        f"{name}_input_shape": list(arr.shape),
        f"{name}_input_dtype": str(arr.dtype),
    }
    ...
    if arr.ndim == 1:
        if arr.size % 3 != 0:
            raise ValueError(
                f"{name} must contain xyz triples; got flat shape {arr.shape}"
            )
        arr = arr.reshape(-1, 3)
        diagnostics[f"{name}_reshape"] = "flat_xyz_triples"
    elif arr.ndim == 2:
        if arr.shape[1] == 3:
            pass
        elif arr.shape[0] == 3:
            arr = arr.T
            diagnostics[f"{name}_reshape"] = "transposed_3_by_n"
        else:
            raise ValueError(f"{name} must have shape (N, 3); got {arr.shape}")
    elif arr.ndim >= 3 and arr.shape[-1] >= 3:
        arr = arr[..., :3].reshape(-1, 3)
        diagnostics[f"{name}_reshape"] = "image_xyz"
    else:
    ...
    if dropped:
        arr = arr[finite]
        diagnostics[f"{name}_dropped_nonfinite"] = dropped

    diagnostics[f"{name}_shape"] = list(arr.shape)
    diagnostics[f"{name}_count"] = int(arr.shape[0])
...
def init_contact_graspnet(device: str = "cuda", checkpoint_path: str | None = None) -> Any:
    ...
    def plan(
        ...
        try:
            ...
            data = resp.json()
            plan.last_diagnostics = data.get("diagnostics", {})
            # end_time = time.time()
...
def init_contact_graspnet_point_clouds() -> Any:
    ...
    def plan_point_clouds(
        ...
        try:
            pc_full, full_diagnostics = _normalize_point_cloud_for_service(
                pc_full, "pc_full"
            )
            pc_segment, segment_diagnostics = _normalize_point_cloud_for_service(
