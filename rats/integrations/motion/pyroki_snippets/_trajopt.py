...
def solve_trajopt(
    ...
    timesteps: int,
    dt: float,
    start_cfg: ArrayLike | None = None,
    end_cfg: ArrayLike | None = None,
) -> ArrayLike:
    ...
    # 1. Solve IK for the start and end poses (skip if joint configs provided).
    target_link_index = robot.links.names.index(target_link_name)
    if start_cfg is None or end_cfg is None:
        start_cfg_solved, end_cfg_solved = solve_iks_with_collision(
            robot=robot,
            coll=robot_coll,
        ...
        if start_cfg is None:
            start_cfg = start_cfg_solved
        if end_cfg is None:
            end_cfg = end_cfg_solved
    else:
        start_cfg = jnp.array(start_cfg)
        end_cfg = jnp.array(end_cfg)
