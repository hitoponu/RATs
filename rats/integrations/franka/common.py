...
def solve_ik_with_convergence(
    ...
    When ``obstacles`` is provided, forwarded to ``ik_solve_fn`` so the
    pyroki server routes through ``solve_ik_with_collision`` and the
    returned configuration avoids both self-collisions and the listed
    world geometry.
    ...
    Args:
        ...
        obstacles: Optional list of obstacle dicts (halfspace/sphere/capsule/box).
