...
class FrankaLiberoPrivilegedApi(ApiBase):
    ...
    def get_object_pose(self, object_name: str) -> tuple[np.ndarray, np.ndarray]:
        ...
        CAVEAT — the returned position is the object body's MuJoCo origin.
        For tall or articulated fixtures (stove, cabinet, drawer, shelf,
        rack) this is rarely the interaction surface: the burner top,
        drawer interior, shelf surface can sit 5-20 cm offset from the
        body origin. If your goal is "place X on Y" and placing at
        `get_object_pose(Y)[0] + small_z` keeps failing with reward=0,
        the reference point itself is probably wrong — do NOT keep
        shrinking the Z offset. Instead, re-localize the actual surface
        from a visual anchor:
          - `get_observation()['robot0_eye_in_hand']` → wrist RGB + depth
          - `get_observation()['agentview']` → fixed-camera RGB + depth
        Each view has `intrinsics` and `pose_mat` so a pixel on the
        target surface can be projected to world coordinates. Use that
        as the placement Z (and XY if the body origin is also offset).
    ...
    def get_all_object_poses(self) -> dict[str, tuple[np.ndarray, np.ndarray]]:
        ...
        The same body-origin caveat from get_object_pose applies here — each
        pose is the object's MuJoCo origin, not the interaction surface.
        For tall fixtures, use a wrist/agentview depth+intrinsics to localize
        the surface instead of relying on pose alone.
