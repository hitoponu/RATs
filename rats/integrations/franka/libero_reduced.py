...
from rats.utils.visualization_utils import draw_molmo_point, overlay_segmentation_masks
...
class FrankaLiberoApiReduced(ApiBase):
    ...
    def __init__(self, env: BaseEnv, grasp_backend: str = "graspnet") -> None:
        super().__init__(env)
    ...
    def segment_sam3_point_prompt(
        ...
        masks = [r["mask"] for r in results[:5] if r.get("mask") is not None]
        if masks:
            ...
            self._log_step_update(
                text=(
                    f"Returned {len(results)} mask(s), best score: {best_score:.3f}; "
                    f"visualized top {len(masks)}"
                ),
                images=vis,
            )
        else:
    ...
    def segment_sam3_text_prompt(
        ...
        self._log_step(
            ...
            f"Running SAM3 text-prompt: '{text_prompt}'",
            images=rgb,
        )
        ...
        if masks:
            ...
            self._log_step_update(text=f"Returned {len(results)} result(s) without valid masks.")
        return results
    ...
    def point_prompt_molmo(
        ...
        try:
            if all(v is not None and None not in v for v in result.values()):
                ...
                self._log_step_update(text=f"Result: {result}")
        except Exception:
            self._log_step_update(text=f"Result: {result}")
        return result
