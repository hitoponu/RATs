...
from rats.utils.depth_utils import (
...
def _get_curobo_api():
    ...
    if _curobo_api is None:
        from rats.integrations.motion import curobo_api as _mod
        _curobo_api = _mod
    return _curobo_api
...
class FrankaLiberoApi(ApiBase):
    ...
    def __init__(self, env: BaseEnv, use_sam3: bool = True, grasp_backend: str = "graspnet") -> None:
        super().__init__(env)
    ...
    def get_object_3d_points_and_masks_from_language(
        ...
        if use_multiview and self.wrist_camera_name in camera_data:
            ...
            # Find intersection between the two clouds. We only consume
            # ``min_distances.min() < threshold`` — i.e. "is ANY pair
            # within 1 cm?" — so the original full pairwise tensor
            # ``agent_pts_3d[:, None, :] - wrist_pts_3d[None, :, :]`` is
            # an O(N·M·3·8B) memory bomb. Observed in libero_main_30iter
            # iter 28: 218 698 × 305 537 × 3 × 8 ≈ 1.46 TiB → numpy
            # ArrayMemoryError, attempt crash. KDTree answers the same
            # "any neighbor within threshold" question in O((N+M) log M).
            if len(wrist_pts_3d) > 0 and len(agent_pts_3d) > 0:
                ...
                try:
                    ...
                    matched = bool(
                        np.any(np.isfinite(dists) & (dists < threshold))
                    )
                except Exception:
                    # Chunked numpy fallback. Memory ≈ chunk · M · 24 B
                    # (3 floats × 8 B). chunk=64 keeps it under ~1 GB
                    # even when M is hundreds of thousands.
                    chunk = 64
