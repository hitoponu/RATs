...
def _worker_loop_with_setup(
    ...
    try:
        ...
        close_fn = getattr(state[0] if isinstance(state, tuple) and state else state, "close", None)
...
def run_parallel_with_setup(
    ...
    *,
    ...
    Args:
        ...
        mp_start_method: Multiprocessing start method (defaults to ``"spawn"`` for
            compatibility with CUDA + Mujoco setups).
        trial_timeout_seconds: Max wall-clock seconds to wait for a single
            result before assuming a worker is stuck and killing all remaining
            workers.  Defaults to 3600 (1 hour).
    ...
    try:
        ...
        while collected < expected:
            try:
                ...
                print(
                    f"WARNING: Timed out after {trial_timeout_seconds:.0f}s "
                    f"waiting for a trial result. {remaining} trial(s) still "
                    f"outstanding — killing workers."
                )
                break
