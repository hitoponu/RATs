...
from rats.llm.client import (  # noqa: F401
    CLAUDE_MODELS,
    ENSEMBLE_CONFIGS,
    GPT_MODELS,
    OPENROUTER_MODELS,
...
def _load_config(args: LaunchArgs) -> tuple[Any, dict[str, Any], list]:
    ...
    merged_config = {
        ...
        "multi_turn_limit": configs_dict.get("multi_turn_limit", 10),
        "evolve_skill_library": configs_dict.get("evolve_skill_library", False),
        "skill_library_path": configs_dict.get("skill_library_path", None),
        "seed_skill_library_path": configs_dict.get("seed_skill_library_path", None),
        "seed_skill_library_select": configs_dict.get("seed_skill_library_select", False),
        "seed_skill_library_select_model": configs_dict.get("seed_skill_library_select_model", None),
        "seed_skill_library_select_temperature": configs_dict.get("seed_skill_library_select_temperature", 0.0),
        "seed_skill_library_select_max_tokens": configs_dict.get("seed_skill_library_select_max_tokens", 32768),
        "seed_skill_library_select_reasoning_effort": configs_dict.get("seed_skill_library_select_reasoning_effort", "low"),
        "seed_skill_library_select_top_k": configs_dict.get("seed_skill_library_select_top_k", None),
        "seed_skill_library_select_io_log": configs_dict.get("seed_skill_library_select_io_log", None),
        # Whether to exec the seeded skill bodies into the policy
        # writer's namespace (default True). Set False to match
        # capx-baseline's v2 planner mode (docs in prompt only,
        # no namespace bind — LLM inline-borrows or calls verbatim
        # if it wants to).
        "seed_skill_library_inject_namespace": configs_dict.get("seed_skill_library_inject_namespace", True),
        "task_name": configs_dict.get("task_name", None),
        "rats_enabled": configs_dict.get("rats", {}).get("enabled", False),
        "rats_scene_model": configs_dict.get("rats", {}).get("fixed_scene_model", None),
        "rats_primitive_api": configs_dict.get("rats", {}).get("primitive_api", "v1"),
        "rats_writer_model": configs_dict.get("rats", {}).get("writer_model", args.model),
        "rats_writer_server_url": configs_dict.get("rats", {}).get("writer_server_url", args.server_url),
        "rats_writer_api_key": configs_dict.get("rats", {}).get("writer_api_key", args.api_key),
        "rats_writer_temperature": configs_dict.get("rats", {}).get("writer_temperature", args.temperature),
        "rats_writer_max_tokens": configs_dict.get("rats", {}).get("writer_max_tokens", args.max_tokens),
        "rats_writer_reasoning_effort": configs_dict.get("rats", {}).get("writer_reasoning_effort", args.reasoning_effort),
        "rats_diagnoser_model": configs_dict.get("rats", {}).get("diagnoser_model", args.visual_differencing_model),
        "rats_diagnoser_server_url": configs_dict.get("rats", {}).get("diagnoser_server_url", args.visual_differencing_model_server_url),
        "rats_diagnoser_api_key": configs_dict.get("rats", {}).get("diagnoser_api_key", args.visual_differencing_model_api_key),
        "rats_diagnoser_temperature": configs_dict.get("rats", {}).get("diagnoser_temperature", 0.2),
        "rats_diagnoser_max_tokens": configs_dict.get("rats", {}).get("diagnoser_max_tokens", 1024),
        "rats_diagnoser_reasoning_effort": configs_dict.get("rats", {}).get("diagnoser_reasoning_effort", args.reasoning_effort),
        "rats_use_orchestrator": configs_dict.get("rats", {}).get("use_orchestrator", False),
        "rats_stop_after_writer": configs_dict.get("rats", {}).get("stop_after_writer", False),
    }

    env_factory = _apply_rats_env_overrides(env_factory, merged_config)
    return env_factory, merged_config, api_servers




def _apply_rats_env_overrides(env_factory: dict[str, Any], merged_config: dict[str, Any]) -> dict[str, Any]:
    updated = copy.deepcopy(env_factory)
    if not merged_config.get("rats_enabled", False):
        return updated

    cfg = updated.get("cfg", {})
    low_level = cfg.get("low_level")
    if isinstance(low_level, dict) and low_level.get("_target_") == "rats.envs.simulators.molmospaces.FrankaMolmoSpacesEnv":
        raise ValueError(
            "rats.enabled is unsupported for MolmoSpaces phase 1. "
            "Use scripts/run_rats.py or CaP-X launch/web flows without the embedded rats_enabled runtime."
        )
    if isinstance(low_level, dict) and low_level.get("_target_") == "rats.envs.simulators.r1pro_b1k.R1ProBehaviourLowLevel":
        primitive_api = merged_config.get("rats_primitive_api", "v1")
        if primitive_api == "legacy":
            cfg["apis"] = ["R1ProControlApi"]
        elif primitive_api == "v1":
            cfg["apis"] = ["R1ProPrimitivesV1Api"]
        else:
            raise ValueError(f"Unsupported rats primitive api: {primitive_api}")
    updated["cfg"] = cfg
    return updated
