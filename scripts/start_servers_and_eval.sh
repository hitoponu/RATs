...
if [ "$all_up" = false ]; then
    ...
    if ! curl -sf -o /dev/null --connect-timeout 2 http://127.0.0.1:8114/ 2>/dev/null; then
        ...
        nohup python -m rats.serving.launch_sam3_server --device cuda --port 8114 --host 127.0.0.1 > logs/sam3.log 2>&1 &
    fi

    # GraspNet server (GPU)
    if ! curl -sf -o /dev/null --connect-timeout 2 http://127.0.0.1:8115/ 2>/dev/null; then
        echo "Starting GraspNet on port 8115..."
        nohup python -m rats.serving.launch_contact_graspnet_server --port 8115 --host 127.0.0.1 > logs/graspnet.log 2>&1 &
    fi

    # PyRoKi server (CPU)
    if ! curl -sf -o /dev/null --connect-timeout 2 http://127.0.0.1:8116/ 2>/dev/null; then
        echo "Starting PyRoKi on port 8116..."
        nohup python -m rats.serving.launch_pyroki_server --port 8116 --host 127.0.0.1 --robot panda_description --target-link panda_hand > logs/pyroki.log 2>&1 &
    fi
    ...
    if ! curl -sf -o /dev/null --connect-timeout 2 http://127.0.0.1:8110/health 2>/dev/null; then
        ...
        nohup python -m rats.serving.openrouter_server --key-file .openrouterkey --port 8110 > logs/nv_server.log 2>&1 &
    fi
...
nohup python -m rats.envs.scripts.run_libero_batch \
